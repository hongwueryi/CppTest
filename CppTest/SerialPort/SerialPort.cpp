...
int CSerial::GetLegelPortName( LPCTSTR lpPort, LPTSTR stOut, int len )
{
    int iRet = 0, iFirst = -1, portNo = 0;

    int i = 0, inLen = _tcslen( lpPort );

    for ( i = 0; i < inLen; i++ )
    {
        if ( _istdigit( lpPort[i] ) )
        {
            iRet++;
    ...
    if ( stOut )
    {
        if ( iRet >= 2 )
        {
            portNo = _ttoi( lpPort + iFirst );
            _stprintf_s( stOut, len, _T( "\\\\.\\COM%d" ), portNo );
        }
        else if ( iRet == 1 )
        {
            _stprintf_s( stOut, len, _T( "%s" ), lpPort );
        }
        else
        {
            //WriteLogD("CSerial::GetLegelPortName::parse serial name error");
            std::cout << "CSerial::GetLegelPortName::parse serial name error";
        }
    }
...
LONG CSerial::Open( LPCTSTR lpszDevice, DWORD dwInQueue, DWORD dwOutQueue, bool fOverlapped )
{
    ...
    m_lLastError = ERROR_SUCCESS;
    int nPortNumLen = -1;
    TCHAR stPortName[MAX_PATH] = {0};
    nPortNumLen = GetLegelPortName( lpszDevice, stPortName, MAX_PATH - 1 );

    //if (1 == nPortNumLen)
    //{
    //	_tcscpy_s(stPortName, lpszDevice);
    //}
    ...
    if ( m_hFile )
    {
        ...
        std::cout << "CSerial::Serial hasn't been open again: " << ws2str( stPortName ).c_str();
        return m_lLastError;
    }

    // Open the device
    m_hFile = ::CreateFile( stPortName,
                            GENERIC_READ | GENERIC_WRITE,
                            0,
                            0,
                            OPEN_EXISTING,
    ...
    if ( m_hFile == INVALID_HANDLE_VALUE )
    {
        ...
        std::cout << "CSerial::Open failed Serial: " << ws2str( stPortName ).c_str();
        return m_lLastError;
    }

    std::cout << "Open success Serial: " << ws2str( stPortName ).c_str();
#ifndef SERIAL_NO_OVERLAPPED
...
LONG CSerial::Setup( EBaudrate eBaudrate, EDataBits eDataBits, EParity eParity, EStopBits eStopBits, int fDtrControl)
{
    // Reset error state
    m_lLastError = ERROR_SUCCESS;
...
LONG CSerial::SetupHandshaking( EHandshake eHandshake )
{
    ...
    switch ( eHandshake )
    {
        ...
        case EHandshakeSoftware:
            ...
            dcb.fRtsControl = RTS_CONTROL_DISABLE;		// Enable RTS (Ready To Send)
            break;
        ...
        if ( lpOverlapped == &ovInternal )
        {
            ...
            switch ( ::WaitForSingleObject( lpOverlapped->hEvent, dwTimeout ) )
            {
                case WAIT_OBJECT_0:
                    ...
                    // The overlapped operation has completed
                    if ( !::GetOverlappedResult( m_hFile, lpOverlapped, pdwWritten, TRUE ) )
                    {
                        // Set the internal error code
                        m_lLastError = ::GetLastError();
...
HANDLE CSerial::GetCommHandle( void )
{
    return m_hFile;
}
